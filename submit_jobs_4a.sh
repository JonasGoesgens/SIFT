...
temp_file=$(mktemp "/tmp/${input_file%.txt}_line${line_index}.XXXXXX")
trap 'rm -f "$temp_file"' EXIT

sed -n "$((line_index + 1))p" "$input_dir/$input_file" > "$temp_file"

apptainer run --bind .:/sift --bind /tmp:/tmp ../sift-container.sif /sift/generate_arg_mask.py -br "$temp_file" -p 10
